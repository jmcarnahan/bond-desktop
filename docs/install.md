...
- **An Apple silicon Mac** — M1 or newer. Intel Macs are not supported, and
  Bond says so on its second screen rather than failing later.
- **macOS 12 or newer.**
...
2. **Your Mac** — the chip, the memory and the macOS version Bond found, and
   whether the models will run here. An Intel Mac stops at this screen. Too
   little memory for the writing model is a warning, not a refusal.
